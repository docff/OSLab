...
static int virtio_crypto_pci_init(VirtIOPCIProxy *vpci_dev)
{
    VirtIOCryptoPCI *dev = VIRTIO_CRYPTO_PCI(vpci_dev);
    DeviceState *vdev = DEVICE(&dev->vdev);
    DeviceState *proxy = DEVICE(vpci_dev);
    char *bus_name;
...
static void virtio_crypto_pci_instance_init(Object *obj)
{
    VirtIOCryptoPCI *dev = VIRTIO_CRYPTO_PCI(obj);
	DEBUG_IN();
    object_initialize(&dev->vdev, sizeof(dev->vdev), "virtio-crypto");
    object_property_add_child(obj, "virtio-backend", OBJECT(&dev->vdev), NULL);
}
...
static const TypeInfo virtio_crypto_pci_info = {
    ...
    .instance_init = virtio_crypto_pci_instance_init,
